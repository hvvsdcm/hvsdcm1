...
try {
  ...
  await admin.route('**/api/**', async route => {
    ...
    else if (path === '/api/admin/users/1/reset-password') {
      const input = route.request().postDataJSON();
      assert.equal(input.password, input.confirmPassword); resets++; body = { ok: true };
  ...
  await adminPage.locator('.reset-user-password').click();
  await adminPage.fill('#resetNewPassword', 'fixture-password-A');
  await adminPage.fill('#resetConfirmPassword', 'fixture-password-B');
  await adminPage.click('#submitPasswordReset');
  checked(resets === 0, 'mismatched reset does not reach server');
  checked(await adminPage.locator('#resetPasswordError').textContent().then(x => x.includes('일치')), 'mismatch feedback');
  await adminPage.fill('#resetConfirmPassword', 'fixture-password-A');
  await adminPage.click('#submitPasswordReset');
  await adminPage.locator('#userPasswordDialog').waitFor({ state: 'hidden' });
  checked(resets === 1, 'single authenticated reset action');
  checked(await adminPage.locator('#resetNewPassword').inputValue() === '', 'password inputs cleared after reset');
  checked(await adminPage.locator('#userResetStatus').textContent().then(x => x.includes('유지')), 'progress preservation feedback');
  await adminPage.locator('.reset-user-password').click();
  if (artifacts) await adminPage.screenshot({ path: join(artifacts, 'admin-reset-mobile.png'), fullPage: true });
  const bounds = await adminPage.locator('#userPasswordDialog').boundingBox();
  checked(bounds && bounds.x >= 0 && bounds.x + bounds.width <= 391, 'mobile reset dialog fits screen');
  await adminPage.click('#cancelPasswordReset');
