...
// ---- 정시 진단 화면 ------------------------------------------------------------
// app.js는 IIFE이고 렌더러를 window.IPSI_RENDER에 얹는다. data.js·engine.js를 먼저 같은 컨텍스트에
// 평가한 뒤 app.js를 그대로 평가하고, 고정 표본 상태로 목표 학과 화면을 그린다.
export const IPSI_APP_SOURCE = 'ipsi/assets/js/app.js';
export function renderIpsiScreen(state) {
  const store = new Map();
  const context = {};
  context.window = context;
  ...
  context.document = stubDocument(store);
  context.location = {
    href: 'https://hvsdcm1.xyz/ipsi/', pathname: '/ipsi/', search: '', hash: '',
    replace() { throw new Error(`${IPSI_APP_SOURCE}: the login gate fired inside the snapshot sandbox`); },
    assign() {}, reload() {},
  };
  context.localStorage = {
    store: new Map([['hvsdcm.token', 'gate-token']]),
  ...
  context.console = { log() {}, warn() {}, error() {} };
  vm.createContext(context);
  for (const file of ['ipsi/assets/js/data.js', 'ipsi/assets/js/engine.js', IPSI_APP_SOURCE]) {
    vm.runInContext(readSource(file), context, { filename: file });
  }
  const renderers = context.IPSI_RENDER;
  if (typeof renderers?.renderTarget !== 'function' || typeof renderers?.defaultState !== 'function') {
    throw new Error(`${IPSI_APP_SOURCE}: renderTarget/defaultState are not reachable — the ipsi snapshot sandbox is broken`);
  }
  const merged = { ...renderers.defaultState(), ...state };
  const body = renderers.renderTarget(merged, context.IPSI_DATA, context.IPSI_ENGINE);
  if (!body.includes('list-group') || !body.includes('view-head')) {
    throw new Error(`${IPSI_APP_SOURCE}: the ipsi target screen rendered without its group-list or header contracts`);
  }
  return { body };
}
